...
class TransactionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final amountColor = isOut ? scheme.error : const Color(0xFF2E7D32); // Green for income
  ...
  (IconData, String, Color) _metaForType(String type, BuildContext context) {
    switch (type) {
      case 'DAILY_PAYMENT':
        return (Icons.savings_outlined, 'Daily Payment', const Color(0xFF2E7D32)); // Green
      case 'DEPOSIT':
        return (Icons.add_circle_outline, 'Deposit', const Color(0xFF1565C0)); // Blue
      case 'WITHDRAW_REQUEST':
        return (Icons.request_page_outlined, 'Withdraw Requested', const Color(0xFFF57C00)); // Amber
      case 'WITHDRAW_APPROVE':
        return (Icons.check_circle_outline, 'Withdraw Approved', const Color(0xFFC62828)); // Red
      case 'WITHDRAW_REJECT':
        return (Icons.cancel_outlined, 'Withdraw Rejected', Colors.grey);
      case 'ADJUSTMENT':
        return (Icons.tune_outlined, 'Adjustment', const Color(0xFF00897B)); // Teal
      default:
